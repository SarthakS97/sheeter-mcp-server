...
async function callSheeterAPI(config, endpoint, options = {}) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            switch (name) {
                ...
                case "read_sheet": {
                    const { sheetId, range = "A:Z", majorDimension = "ROWS", valueRenderOption = "FORMATTED_VALUE" } = args;
                    const queryParams = new URLSearchParams({ range, majorDimension, valueRenderOption });
                    const result = await callSheeterAPI(config, `/api/sheets/${sheetId}?${queryParams}`);
                    const dataText = result.data?.length > 0
                        ? result.data.slice(0, 10).map((row, idx) => `Row ${idx + 1}: ${Object.entries(row).map(([key, value]) => `${key}: ${value}`).join(", ")}`).join("\n") + (result.data.length > 10 ? `\n... and ${result.data.length - 10} more rows` : "")
                        : "No data found";
                    return {
                        content: [
                            {
                ...
                case "write_sheet": {
                    const { sheetId, range, values, valueInputOption = "USER_ENTERED" } = args;
                ...
                case "append_to_sheet": {
                    const { sheetId, values, valueInputOption = "USER_ENTERED" } = args;
                ...
                case "batch_get_ranges": {
                    const { sheetId, ranges, majorDimension = "ROWS", valueRenderOption = "FORMATTED_VALUE" } = args;
                    ...
                    const rangeResults = result.valueRanges?.map((vr, idx) => `Range ${idx + 1} (${vr.range}): ${vr.values?.length || 0} rows`).join("\n") || "No ranges returned";
                    return {
                        content: [
                            {
                                type: "text",
                ...
                case "batch_update_ranges": {
                    const { sheetId, data, valueInputOption = "USER_ENTERED" } = args;
                ...
                case "get_sheet_metadata": {
                    const { sheetId } = args;
                    const result = await callSheeterAPI(config, `/api/sheets/${sheetId}/metadata`);
                    const sheetsInfo = result.sheets?.map(sheet => `• ${sheet.title} (ID: ${sheet.sheetId}, ${sheet.gridProperties.rowCount} rows × ${sheet.gridProperties.columnCount} cols)`).join("\n") || "No sheets found";
                    return {
                        content: [
                            {
                                type: "text",
                ...
                case "delete_rows": {
                    const { sheetId, startIndex, endIndex, sheetTabId = 0 } = args;
